 ...
 *
 */
public class UsermodeQueryRunnable implements Runnable {
	
	private WifiReadDataFormat formatReadMessage=new WifiReadDataFormat(0x200000AC,4);
	 ...
	 */
	public UsermodeQueryRunnable(Activity targetActivity, ListView tempListView) {
		try{
			existFlag=true;
		...
		pointShowRunnable=new Runnable() {
			...
			public void run() {
				...
				for (int i =0; i < devicelist.length; i++) {
					int state=(int)((getData[(devicelist[i]-1)/8]>>((devicelist[i]-1)%8))&0x01);
					if(state==1){
						deviceTextViews[i].setText("ON");
						deviceTextViews[i].setBackgroundColor(Color.BLUE);
	 				}else{
	 					deviceTextViews[i].setText("OFF");
	 					deviceTextViews[i].setBackgroundColor(Color.GRAY);
	 				}
				}
			}
		};
